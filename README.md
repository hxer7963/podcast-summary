...
脚本会创建 `episode_dir`、`README.md` 和火山原始结果；若本机已有 `jq` 会同时写带 speaker 标签、不带 timestamp 的 `transcript.md`，否则 agent 直接格式化 `volc-response.json` 中的 utterances，不为此安装新工具。时间明细仍完整保留在 `volc-response.json`。RSS/Apple/Spotify 抓取和视频字幕依赖只在对应路径首次需要时安装：`bash install.sh --with-fetch` 或 `bash install.sh --with-subtitle`。
